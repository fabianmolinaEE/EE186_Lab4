...
int main(void)
{
  ...
  while (1)
  {
	dac_val = 0;
    //first increment
    for (dac_val = 0; dac_val <= 4095; dac_val += 100) //increase by 100 each iteration.
    {
        // Set DAC output
        ...
        dac_voltage = (dac_val / 4095.0f) * 3.3f;

        // Print the results
        printf("DAC Voltage: %.2f V, ADC Voltage: %.2f V, Difference: %.2f V \r\n", dac_voltage, adc_voltage, (dac_voltage - adc_voltage));
        HAL_Delay(100);
    }
    //then decrement
    for (dac_val = 4095; dac_val >= 0; dac_val -= 100) //decrease by 100 each iteration.
    {
        // Set DAC output
        HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dac_val);
        HAL_DAC_Start(&hdac1, DAC_CHANNEL_1);
